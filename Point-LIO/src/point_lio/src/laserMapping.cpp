...
{
    const double odom_stamp = publish_odometry_without_downsample ? time_current : lidar_end_time;
    static double last_publish_stamp = -1.0;
    if (publish_odometry_frequency > 0.0 && last_publish_stamp >= 0.0)
    {
        const double min_interval = 1.0 / publish_odometry_frequency;
        if (odom_stamp >= last_publish_stamp && odom_stamp - last_publish_stamp < min_interval)
        {
            return;
        }
    }
    last_publish_stamp = odom_stamp;

    odomAftMapped.header.frame_id = "camera_init";
    odomAftMapped.child_frame_id = "body";
    odomAftMapped.header.stamp = ros::Time().fromSec(odom_stamp);
